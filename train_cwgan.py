...
def get_config(max_g_fc_layer_num, max_d_fc_layer_num):
    ...
    param_dict = {
        ...
        'normal_dim': {'distribution': 'int_uniform', 'min': 75, 'max': 120},
        'n_critic': {'distribution': 'int_uniform', 'min': 1, 'max': 2},
        'clip': {'distribution': 'uniform', 'min': 0.05, 'max': 0.15},
        'embedding_dim': {'distribution': 'int_uniform', 'min': 75, 'max': 150}
    }
    ...
    sweep_config['parameters'].update(
        {"g_learning_rate": {'distribution': 'uniform', 'min': 0.001, 'max': 0.05},
         ...
         'epoch_num': {'distribution': 'int_uniform', 'min': 10, 'max': 30}})
    return sweep_config
...
def train(config=None):
    with wandb.init(config=config):
        ...
        log_interval = 50 * config['n_critic']
...
def main():
    ...
    sweep_id = wandb.sweep(get_config(30, 10), project="ex3_CWGAN3",
